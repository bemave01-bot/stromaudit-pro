...
async def get_smard_price() -> dict:
    ...
    fallback = {
        ...
        "source":           f"Fallback (Ø DE 2025: {CONFIG['fallback_marktpreis_eur_mwh']:.2f} EUR/MWh) – SMARD nicht erreichbar",
        "is_fallback":      True,
        "data_points_7d":   0,
        "cached":           False,
    }
    ...
    for attempt in range(1, SMARD_MAX_RETRIES + 1):
        try:
            async with httpx.AsyncClient(
                ...
                # Timestamp-validatie: niet ouder dan 48 uur
                ...
                if age_hours > 48:
                    raise ValueError(f"SMARD-data te oud: {age_hours:.0f} uur")
...
def generiere_html(
    ...
    data_age     = f"{markt.get('data_age_hours', '—')} Stunden" if not markt.get("is_fallback") else "—"
