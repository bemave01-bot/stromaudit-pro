...
# ─────────────────────────────────────────────────────────────────────────────
# DUITSE GETALNOTATIE (punt als duizendtal, komma als decimaal)
# ─────────────────────────────────────────────────────────────────────────────
def de_num(value: float, decimals: int = 2) -> str:
    """Formatteert een getal naar Duitse notatie: 125.000,50"""
    formatted = f"{value:,.{decimals}f}"          # Engels: 125,000.50
    # Wissel punt en komma om
    formatted = formatted.replace(",", "X").replace(".", ",").replace("X", ".")
    return formatted

def de_eur(value: float) -> str:
    return de_num(value, 2)

def de_kwh(value: float) -> str:
    return de_num(value, 0)

def de_ct(value: float, decimals: int = 3) -> str:
    return de_num(value, decimals)
...
<table>
  ...
  <tr><td><strong>Projekt-Referenz</strong></td><td colspan="3"><strong>{data['projekt_referenz']}</strong></td></tr>
</table>
</div>
...
<div class="kpi-grid">
  ...
  <div class="kpi"><div class="lbl">Leistungskosten</div>
    <div class="val">{de_kwh(kalk['leistungskosten_netto_eur'])} €</div><div class="sub">{de_num(kw,1)} kW × {t['leistungspreis_eur_kw']:.0f} €/kW</div></div>
  ...
  <div class="kpi"><div class="lbl">Day-Ahead Marktpreis</div>
    <div class="val">{de_num(markt['raw_eur_mwh'],1)}</div><div class="sub">EUR/MWh (SMARD live)</div></div>
...
<table>
  ...
  <tr><td>Day-Ahead Marktpreis (SMARD)</td><td class="r">{de_eur(t['marktpreis_eur_kwh']*1000)} EUR/MWh</td><td>EPEX Spot / EnWG §1</td></tr>
